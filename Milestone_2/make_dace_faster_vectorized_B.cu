...
#pragma unroll
    for (int i = 0; i < TIMES; i++) {
        ...
        if ((THREADBLOCK_TILE_M * LOAD_K % THREADS == 0 || (i * THREADS + threadIdx.x) < THREADBLOCK_TILE_M * LOAD_K)) {
            ...
            if ((M % THREADBLOCK_TILE_M == 0 || global_i < M) && (!K_CHECK || global_j < K) && (!THREADBLOCK_TILE_K_CHECK || cta_k + shared_j < THREADBLOCK_TILE_K)) {
                a = A[global_i * lda + global_j];
            } else {
                a = 0;
            }
            (*A_Shared)[A_Shared_Offset + shared_i + (THREADBLOCK_TILE_M + A_OFFSET) * shared_j] = a;
        }
    }
}
...
*/
template<bool K_CHECK, bool THREADBLOCK_TILE_K_CHECK>
__device__ __inline__ void load_A_Global_Vector4(TYPE (* __restrict__ A_Shared)[2 * (THREADBLOCK_TILE_M + A_OFFSET) * LOAD_K], const TYPE* __restrict__ A, const int lda, const int cta_k, const int block_idx_y, const int A_Shared_Offset) {
...
#pragma unroll
    for (int i = 0; i < TIMES; i++) {
        ...
        if ((THREADBLOCK_TILE_M * LOAD_K_VECTOR % THREADS == 0 || (i * THREADS + threadIdx.x) < THREADBLOCK_TILE_M * LOAD_K_VECTOR)) {
            ...
            // If the tiles are not perfect multiples we need to make this checks.
            if ((M % THREADBLOCK_TILE_M == 0 || global_i < M) && (!K_CHECK || global_j < K) && (!THREADBLOCK_TILE_K_CHECK || cta_k + shared_j * VECTORCOUNT < THREADBLOCK_TILE_K)) {
            ...
            (*A_Shared)[A_Shared_Offset + shared_i + (THREADBLOCK_TILE_M + A_OFFSET) * (shared_j * VECTORCOUNT + 0)] = a.x;
            (*A_Shared)[A_Shared_Offset + shared_i + (THREADBLOCK_TILE_M + A_OFFSET) * (shared_j * VECTORCOUNT + 1)] = a.y;
            (*A_Shared)[A_Shared_Offset + shared_i + (THREADBLOCK_TILE_M + A_OFFSET) * (shared_j * VECTORCOUNT + 2)] = a.z;
            (*A_Shared)[A_Shared_Offset + shared_i + (THREADBLOCK_TILE_M + A_OFFSET) * (shared_j * VECTORCOUNT + 3)] = a.w;
        }
    }
}
...
*/
template<bool K_CHECK, bool THREADBLOCK_TILE_K_CHECK>
__device__ __inline__ void load_B_Global_Vector4(TYPE (* __restrict__ B_Shared)[2 * LOAD_K * (THREADBLOCK_TILE_N + B_OFFSET)], const TYPE* __restrict__ B, const int ldb, const int cta_k, const int block_idx_x, const int B_Shared_Offset) {
    constexpr int VECTORCOUNT = 4;
...
#pragma unroll
    for (int i = 0; i < TIMES; i++) {
        ...
        if ((THREADBLOCK_TILE_N_VECTOR * LOAD_K % THREADS == 0 || (i * THREADS + threadIdx.x) < THREADBLOCK_TILE_N_VECTOR * LOAD_K)) {
            // If the tiles are not perfect multiples we need to make this checks.
            if ((!K_CHECK || global_i < K) && (N % THREADBLOCK_TILE_N == 0 || global_j < N) && (!THREADBLOCK_TILE_K_CHECK || cta_k + shared_i < THREADBLOCK_TILE_K)) {
                const TYPE* global_pointer = &B[global_i * ldb + global_j];
                VECTORTYPE4 a2 = reinterpret_cast<const VECTORTYPE4*>(global_pointer)[0];

                reinterpret_cast<VECTORTYPE4*>(B_Shared)[B_Shared_Offset / 4 + shared_i * THREADBLOCK_TILE_N_VECTOR + shared_j] = a2;
            } else {
                VECTORTYPE4 zero;
                zero.x = 0;
                zero.y = 0;
                ...
                reinterpret_cast<VECTORTYPE4*>(B_Shared)[B_Shared_Offset / 4 + shared_i * THREADBLOCK_TILE_N_VECTOR + shared_j] = zero;
            }
        }
    }
}
    ...
    for (int i = 0; i < TIMES; i++) {
        ...
        if ((THREADBLOCK_TILE_N_VECTOR * LOAD_K % THREADS == 0 || (i * THREADS + threadIdx.x) < THREADBLOCK_TILE_N_VECTOR * LOAD_K)) {
            ...
            if ((!K_CHECK || global_i < K) && (N % THREADBLOCK_TILE_N == 0 || global_j < N) && (!THREADBLOCK_TILE_K_CHECK || cta_k + shared_i < THREADBLOCK_TILE_K)) {
                ...
                reinterpret_cast<VECTORTYPE2*>(B_Shared)[B_Shared_Offset / 2 + shared_i * THREADBLOCK_TILE_N_VECTOR + shared_j] = a2;
            } else {
                VECTORTYPE2 zero;
                zero.x = 0;
                zero.y = 0;

                reinterpret_cast<VECTORTYPE2*>(B_Shared)[B_Shared_Offset / 2 + shared_i * THREADBLOCK_TILE_N_VECTOR + shared_j] = zero;
            }
        }
    }
}
...
#pragma unroll
    for (int i = 0; i < TIMES; i++) {
        ...
        if ((THREADBLOCK_TILE_N * LOAD_K % THREADS == 0 || (i * THREADS + threadIdx.x) < THREADBLOCK_TILE_N * LOAD_K)) {
            ...
            (*B_Shared)[B_Shared_Offset + shared_i * THREADBLOCK_TILE_N + shared_j] = a;
        }
    }
}
...
__device__ __inline__ void load_B_Global(TYPE (* __restrict__ B_Shared)[2 * LOAD_K * (THREADBLOCK_TILE_N + B_OFFSET)], const TYPE* __restrict__ B, const int ldb, const int cta_k, const int block_idx_x, const int B_Shared_Offset) {
    if (((THREADBLOCK_TILE_N / 4) * LOAD_K) % THREADS == 0 && useVector4) {
        load_B_Global_Vector4<K_CHECK, THREADBLOCK_TILE_K_CHECK>(B_Shared, B, ldb, cta_k, block_idx_x, B_Shared_Offset);
    } else if (((THREADBLOCK_TILE_N / 2) * LOAD_K) % THREADS == 0 && useVector2) {
        load_B_Global_Vector2<K_CHECK, THREADBLOCK_TILE_K_CHECK>(B_Shared, B, ldb, cta_k, block_idx_x, B_Shared_Offset);
    } else if ((THREADBLOCK_TILE_N * LOAD_K) % THREADS == 0) {
        load_B_Global_Single<K_CHECK, THREADBLOCK_TILE_K_CHECK>(B_Shared, B, ldb, cta_k, block_idx_x, B_Shared_Offset);
    } else if (useVector4) {
        load_B_Global_Vector4<K_CHECK, THREADBLOCK_TILE_K_CHECK>(B_Shared, B, ldb, cta_k, block_idx_x, B_Shared_Offset);
    } else if (useVector2) {
        load_B_Global_Vector2<K_CHECK, THREADBLOCK_TILE_K_CHECK>(B_Shared, B, ldb, cta_k, block_idx_x, B_Shared_Offset);
...
*/
__device__ __inline__ void compute_inner(const TYPE (* __restrict__ A_register)[ THREAD_TILE_M], const TYPE (* __restrict__ B_register)[ THREAD_TILE_N], TYPE (*Thread_Tile)[THREAD_TILE_M * THREAD_TILE_N]) {
...
*/
__device__ __inline__ void load_A_Shared(const TYPE (* __restrict__ A_Shared)[2 * (THREADBLOCK_TILE_M + A_OFFSET) * LOAD_K], TYPE (* __restrict__ A_register)[ THREAD_TILE_M], const int k, const int WarpIdy, const int LaneIdy, const int A_Shared_Offset) {
...
// And use one single load in the end, if there is still some rest
    if (THREAD_TILE_M % 2 > 0) {
        ...
        (*A_register)[TIMES * 4 + ADDITIONAL_OFFSET_REGISTER] = (*A_Shared)[A_Shared_Offset + Shared_i + (THREADBLOCK_TILE_M + A_OFFSET) * Shared_j];
    }
}
...
*/
__device__ __inline__ void load_B_Shared(TYPE (* __restrict__ B_Shared)[2 * LOAD_K * (THREADBLOCK_TILE_N + B_OFFSET)], TYPE (* __restrict__ B_register)[ THREAD_TILE_N], const int k, const int WarpIdx, const int LaneIdx, const int B_Shared_Offset) {
...
// And use one single load in the end, if there is still some rest
    if (THREAD_TILE_N % 2 > 0) {
        ...
        (*B_register)[TIMES * 4 + ADDITIONAL_OFFSET_REGISTER] = (*B_Shared)[B_Shared_Offset + Shared_i * (THREADBLOCK_TILE_N + B_OFFSET) + Shared_j];
    }
}
...
#pragma unroll
    for (int j = 0; j < N_TIMES; j++) {
        ...
        if (EPILOGUE_N <= 32) {
            ...
            if (SPLIT_K == 1) {
                ...
                if ((M % THREADBLOCK_TILE_M == 0 || global_i + 1 * THREAD_TILE_Y_HEIGHT < M) && (N % THREADBLOCK_TILE_N == 0 || global_j < N)) {
                    a1 = C[(global_i + 1 * THREAD_TILE_Y_HEIGHT) * ldc + global_j];
                }
                if ((M % THREADBLOCK_TILE_M == 0 || global_i + 2 * THREAD_TILE_Y_HEIGHT < M) && (N % THREADBLOCK_TILE_N == 0 || global_j < N)) {
                    a2 = C[(global_i + 2 * THREAD_TILE_Y_HEIGHT) * ldc + global_j];
                }
                if ((M % THREADBLOCK_TILE_M == 0 || global_i + 3 * THREAD_TILE_Y_HEIGHT < M) && (N % THREADBLOCK_TILE_N == 0 || global_j < N)) {
                    a3 = C[(global_i + 3 * THREAD_TILE_Y_HEIGHT) * ldc + global_j];
                }
            }

            (*Shared)[SHARED_MEM_PER_WARP * WarpId + (threadIdIdy_epilogue * 4 + 0) * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue] = a0;
            (*Shared)[SHARED_MEM_PER_WARP * WarpId + (threadIdIdy_epilogue * 4 + 1) * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue] = a1;
            (*Shared)[SHARED_MEM_PER_WARP * WarpId + (threadIdIdy_epilogue * 4 + 2) * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue] = a2;
            (*Shared)[SHARED_MEM_PER_WARP * WarpId + (threadIdIdy_epilogue * 4 + 3) * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue] = a3;
            ...
            if (SPLIT_K == 1) {
                if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 0 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_1 < N)) {
                    a0 = C[(global_i + 0 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_1];
                }
                if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 1 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_1 < N)) {
                    a1 = C[(global_i + 1 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_1];
                }
                if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 0 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_2 < N)) {
                    a2 = C[(global_i + 0 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_2];
                }
                if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 1 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_2 < N)) {
                    a3 = C[(global_i + 1 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_2];
                }
            }

            (*Shared)[SHARED_MEM_PER_WARP * WarpId + 0 * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue_1] = a0;
            (*Shared)[SHARED_MEM_PER_WARP * WarpId + 1 * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue_1] = a1;
            (*Shared)[SHARED_MEM_PER_WARP * WarpId + 0 * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue_2] = a2;
            (*Shared)[SHARED_MEM_PER_WARP * WarpId + 1 * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue_2] = a3;
        }
            ...
            if (SPLIT_K == 1) {
                ...
                if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 0 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_3 < N)) {
                    a2 = C[(global_i + 0 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_3];
                }
                if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 0 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_4 < N)) {
                    a3 = C[(global_i + 0 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_4];
                }
            }

            (*Shared)[SHARED_MEM_PER_WARP * WarpId + 0 * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue_1] = a0;
            (*Shared)[SHARED_MEM_PER_WARP * WarpId + 0 * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue_2] = a1;
            (*Shared)[SHARED_MEM_PER_WARP * WarpId + 0 * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue_3] = a2;
            (*Shared)[SHARED_MEM_PER_WARP * WarpId + 0 * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue_4] = a3;
        }
        ...
        TYPE* shared_ptr = &(*Shared)[SHARED_MEM_PER_WARP * WarpId + epilogue_i_write * (EPILOGUE_N + EPILOGUE_OFFSET) + epilogue_j_write];
        reinterpret_cast<VECTORTYPE4*>(a_ptr)[0] += BETA * reinterpret_cast<VECTORTYPE4*>(shared_ptr)[0];
    }
    ...
    if (THREAD_TILE_N % 4 >= 2) {
        ...
        // M_THREADS: 32, EPILOGUE_OFFSET: 1
        constexpr int EPILOGUE_OFFSET = (M_THREADS == 1 || M_THREADS == 16) ? 0 : (M_THREADS == 32) ? 0 : (M_THREADS == 4) ? 8 : (M_THREADS == 2) ? 16 : 4;
        ...
        if (EPILOGUE_N <= 32) {
            ...
            (*Shared)[SHARED_MEM_PER_WARP * WarpId + (threadIdIdy_epilogue * 2 + 0) * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue] = a0;
            (*Shared)[SHARED_MEM_PER_WARP * WarpId + (threadIdIdy_epilogue * 2 + 1) * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue] = a1;
        ...
        TYPE* shared_ptr = &(*Shared)[SHARED_MEM_PER_WARP * WarpId + epilogue_i_write * (EPILOGUE_N + EPILOGUE_OFFSET) + epilogue_j_write];
        reinterpret_cast<VECTORTYPE2*>(a_ptr)[0] += BETA * reinterpret_cast<VECTORTYPE2*>(shared_ptr)[0];
    }
    ...
    if (THREAD_TILE_N % 2 > 0) {
        ...
        (*Shared)[SHARED_MEM_PER_WARP * WarpId + (threadIdIdy_epilogue * 1 + 0) * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue] = a0;
        ...
        Thread_Tile[Threadtile_i * THREAD_TILE_N + Threadtile_j] += BETA * (*Shared)[SHARED_MEM_PER_WARP * WarpId + epilogue_i_write * (EPILOGUE_N + EPILOGUE_OFFSET) + epilogue_j_write];
    }
}
...
#pragma unroll
        for (int j = 0; j < N_TIMES; j++) {
            ...
            if (EPILOGUE_N <= 32) {
                ...
                // Load the values
                const TYPE a0 = (*Shared)[SHARED_MEM_PER_WARP * WarpId + (threadIdIdy_epilogue * 4 + 0) * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue];
                const TYPE a1 = (*Shared)[SHARED_MEM_PER_WARP * WarpId + (threadIdIdy_epilogue * 4 + 1) * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue];
                const TYPE a2 = (*Shared)[SHARED_MEM_PER_WARP * WarpId + (threadIdIdy_epilogue * 4 + 2) * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue];
                ...
                if (SPLIT_K == 1 || !ATOMIC_REDUCTION) {
                    if ((M % THREADBLOCK_TILE_M == 0 || global_i + 0 * THREAD_TILE_Y_HEIGHT < M) && (N % THREADBLOCK_TILE_N == 0 || global_j < N)) {
                        C[split_K_OFFSET + (global_i + 0 * THREAD_TILE_Y_HEIGHT) * ldc + global_j] = a0;
                    }
                    if ((M % THREADBLOCK_TILE_M == 0 || global_i + 1 * THREAD_TILE_Y_HEIGHT < M) && (N % THREADBLOCK_TILE_N == 0 || global_j < N)) {
                        C[split_K_OFFSET + (global_i + 1 * THREAD_TILE_Y_HEIGHT) * ldc + global_j] = a1;
                    }
                    if ((M % THREADBLOCK_TILE_M == 0 || global_i + 2 * THREAD_TILE_Y_HEIGHT < M) && (N % THREADBLOCK_TILE_N == 0 || global_j < N)) {
                        C[split_K_OFFSET + (global_i + 2 * THREAD_TILE_Y_HEIGHT) * ldc + global_j] = a2;
                    }
                    if ((M % THREADBLOCK_TILE_M == 0 || global_i + 3 * THREAD_TILE_Y_HEIGHT < M) && (N % THREADBLOCK_TILE_N == 0 || global_j < N)) {
                        C[split_K_OFFSET + (global_i + 3 * THREAD_TILE_Y_HEIGHT) * ldc + global_j] = a3;
                    }
                } else {
                    if ((M % THREADBLOCK_TILE_M == 0 || global_i + 0 * THREAD_TILE_Y_HEIGHT < M) && (N % THREADBLOCK_TILE_N == 0 || global_j < N)) {
                        atomicAdd(&C[(global_i + 0 * THREAD_TILE_Y_HEIGHT) * ldc + global_j], a0);
                    }
                    if ((M % THREADBLOCK_TILE_M == 0 || global_i + 1 * THREAD_TILE_Y_HEIGHT < M) && (N % THREADBLOCK_TILE_N == 0 || global_j < N)) {
                        atomicAdd(&C[(global_i + 1 * THREAD_TILE_Y_HEIGHT) * ldc+ global_j], a1);
                    }
                    if ((M % THREADBLOCK_TILE_M == 0 || global_i + 2 * THREAD_TILE_Y_HEIGHT < M) && (N % THREADBLOCK_TILE_N == 0 || global_j < N)) {
                        atomicAdd(&C[(global_i + 2 * THREAD_TILE_Y_HEIGHT) * ldc + global_j], a2);
                    }
                    if ((M % THREADBLOCK_TILE_M == 0|| global_i + 3 * THREAD_TILE_Y_HEIGHT < M) && (N % THREADBLOCK_TILE_N == 0 || global_j < N)) {
                        atomicAdd(&C[(global_i + 3 * THREAD_TILE_Y_HEIGHT) * ldc + global_j], a3);
                    }
                }
                ...
                // Load the values
                const TYPE a0 = (*Shared)[SHARED_MEM_PER_WARP * WarpId + 0 * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue_1];
                const TYPE a1 = (*Shared)[SHARED_MEM_PER_WARP * WarpId + 1 * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue_1];
                const TYPE a2 = (*Shared)[SHARED_MEM_PER_WARP * WarpId + 0 * (EPILOGUE_N + EPILOGUE_OFFSET)+ threadIdIdx_epilogue_2];
                const TYPE a3 = (*Shared)[SHARED_MEM_PER_WARP * WarpId + 1 * (EPILOGUE_N + EPILOGUE_OFFSET)+ threadIdIdx_epilogue_2];
                ...
                if (SPLIT_K == 1 || !ATOMIC_REDUCTION) {
                    if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 0 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_1 < N)) {
                        C[split_K_OFFSET + (global_i + 0 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_1] = a0;
                    }
                    if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 1 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_1 < N)) {
                        C[split_K_OFFSET + (global_i + 1 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_1] = a1;
                    }
                    if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 0 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_2 < N)) {
                        C[split_K_OFFSET + (global_i + 0 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_2] = a2;
                    }
                    if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 1 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_2 < N)) {
                        C[split_K_OFFSET + (global_i + 1 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_2] = a3;
                    }

                } else {
                    if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 0 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_1 < N)) {
                        atomicAdd(&C[(global_i + 0 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_1], a0);
                    }
                    if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 1 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_1 < N)) {
                        atomicAdd(&C[(global_i + 1 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_1], a1);
                    }
                    if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 0 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_2 < N)) {
                        atomicAdd(&C[(global_i + 0 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_2], a2);
                    }
                    if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 1 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_2 < N)) {
                        atomicAdd(&C[(global_i + 1 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_2], a3);
                    }
                }
                ...
                const TYPE a0 = (*Shared)[SHARED_MEM_PER_WARP * WarpId + 0 * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue_1];
                const TYPE a1 = (*Shared)[SHARED_MEM_PER_WARP * WarpId + 0 * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue_2];
                const TYPE a2 = (*Shared)[SHARED_MEM_PER_WARP * WarpId + 0 * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue_3];
                const TYPE a3 = (*Shared)[SHARED_MEM_PER_WARP * WarpId + 0 * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue_4];
                ...
                if (SPLIT_K == 1 || !ATOMIC_REDUCTION) {
                    ...
                    if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 0 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_2 < N)) {
                        C[split_K_OFFSET + (global_i + 0 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_2] = a1;
                    }
                    if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 0 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_3 < N)) {
                        C[split_K_OFFSET + (global_i + 0 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_3] = a2;
                    }
                    if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 0 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_4 < N)) {
                        C[split_K_OFFSET + (global_i + 0 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_4] = a3;
                    }
                    ...
                    if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 0 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_2 < N)) {
                        atomicAdd(&C[(global_i + 0 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_2], a1);
                    }
                    if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 0 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_3 < N)) {
                        atomicAdd(&C[(global_i + 0 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_3], a2);
                    }
                    if ((M % THREADBLOCK_TILE_M == 0 || (global_i + 0 * THREAD_TILE_Y_HEIGHT) < M) && (N % THREADBLOCK_TILE_N == 0 || global_j_4 < N)) {
                        atomicAdd(&C[(global_i + 0 * THREAD_TILE_Y_HEIGHT) * ldc + global_j_4], a3);
                    }
                }
            }
...
//		constexpr int EPILOGUE_OFFSET = 5;
        ...
        if (THREAD_TILE_N % 4 >= 2) {
            ...
            if (EPILOGUE_N <= 32) {
                ...
                // Load the values
                const TYPE a0 = (*Shared)[SHARED_MEM_PER_WARP * WarpId + (threadIdIdy_epilogue * 2 + 0) * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue];
...
// M_THREADS: 32, EPILOGUE_OFFSET: 0
        ...
        (*Shared)[SHARED_MEM_PER_WARP * WarpId + epilogue_i_write * (EPILOGUE_N + EPILOGUE_OFFSET) + epilogue_j_write] = a_val;
        ...
        const TYPE a0 = (*Shared)[SHARED_MEM_PER_WARP * WarpId + (threadIdIdy_epilogue * 1 + 0) * (EPILOGUE_N + EPILOGUE_OFFSET) + threadIdIdx_epilogue];
...
DACE_DFI void nested_nested_state_1_1_5(const float * input_A, const dace::vec<float, 4> * input_B, float * output, int K, int M, int N) {
    ...
    int ldb = N;
    int ldc = K;
    constexpr int M_WARPS = THREADBLOCK_TILE_M / WARP_TILE_M; // 128 / 32 = 4
    constexpr int N_WARPS = THREADBLOCK_TILE_N / WARP_TILE_N; // 128 / 64 = 2
...
#pragma unroll
    ...
    load_A_Global<A_VECTOR_4_LAST, A_VECTOR_2_LAST, K_CHECK, THREADBLOCK_TILE_K_CHECK>(&A_Shared, input_A, lda, cta_k, block_idx_y, A_Shared_Offset_0);
    // load_B_Global<B_VECTOR_4, B_VECTOR_2, K_CHECK, THREADBLOCK_TILE_K_CHECK>(&B_Shared, input_B, ldb, cta_k, block_idx_x, B_Shared_Offset_0);
    // dace::GlobalToShared2D<float, max(1, num_threads_per_threadblock), 1, 1, size_thread_block_tile_m, size_K_tile, 1, 128, true>(input_A + ((K * size_thread_block_tile_m) * block_idx_y) + cta_k, K, 1, A_Shared + A_Shared_Offset_0);
    dace::GlobalToShared2D<dace::vec<float, 4>, max(1, num_threads_per_threadblock), 1, 1, size_K_tile, size_thread_block_tile_n / 4, 128 / 4, 1, true>(input_B + ((K * cta_k) + (size_thread_block_tile_n * block_idx_x)) / 4, N / 4, 1, (((dace::vec<float, 4> *)B_Shared) + B_Shared_Offset_0 / 4));
...
#pragma unroll
        for (int k = 0; k < LOAD_K; k++) {
            if (k % 2 == 0) {
                ...
                load_A_Shared(&A_Shared, &A_register_1, k, WarpIdy, LaneIdy, A_Shared_Offset_0);
                load_B_Shared(&B_Shared, &B_register_1, k, WarpIdx, LaneIdx, B_Shared_Offset_0);
                // dace::CopyND<float, 1, false, size_thread_tile_m / 2>::template ConstDst<1>::Copy(
                //     A_Shared + (((128 * k + ((size_thread_tile_m / 2) * bitwise_and(right_shift((thread % 32), 1), (warp_height - 1)))) + ((size_warp_tile_m) * ((thread / 32) / num_warps_n))) + A_Shared_Offset_0), A_register_1, 1);
                // dace::CopyND<float, 1, false, size_thread_tile_m / 2>::template ConstDst<1>::Copy(
                //     A_Shared + (((128 * k + ((size_thread_tile_m / 2) * bitwise_and(right_shift((thread % 32), 1), (warp_height - 1)))) + ((size_warp_tile_m) * ((thread / 32) / num_warps_n))) + A_Shared_Offset_0 + 16), A_register_1 + 4, 1);
                // dace::CopyND<float, 1, false, size_thread_tile_n / 2>::template ConstDst<1>::Copy(
                //     B_Shared + ((((128 * k) + (size_thread_tile_n / 2 * bitwise_or(right_shift(bitwise_and((thread % 32), 24), 2), bitwise_and((thread % 32), 1)))) + (size_warp_tile_n * ((thread / 32) % num_warps_n))) + B_Shared_Offset_0), B_register_1, 1);
                // dace::CopyND<float, 1, false, size_thread_tile_n / 2>::template ConstDst<1>::Copy(
                //     B_Shared + ((((128 * k) + (size_thread_tile_n / 2 * bitwise_or(right_shift(bitwise_and((thread % 32), 24), 2), bitwise_and((thread % 32), 1)))) + (size_warp_tile_n * ((thread / 32) % num_warps_n))) + B_Shared_Offset_0 + 32), B_register_1 + 4, 1);
            }
            ...
            if (k == LOAD_K - 1) {
                ...
                load_A_Global<A_VECTOR_4, A_VECTOR_2, (THREADBLOCK_TILE_K * SPLIT_K - K_ > LOAD_K), false>(&A_Shared, input_A, lda, cta_k, block_idx_y, A_Shared_Offset_1);
                ...
                dace::GlobalToShared2D<dace::vec<float, 4>, max(1, num_threads_per_threadblock), 1, 1, size_K_tile, size_thread_block_tile_n / 4, 128 / 4, 1, true>(input_B + ((K * cta_k) + (size_thread_block_tile_n * block_idx_x)) / 4, N / 4, 1, (((dace::vec<float, 4> *)B_Shared) + B_Shared_Offset_1 / 4));
                __syncthreads();
            }
    ...
    for (k_tile = 0; (k_tile < (num_K_tiles - 1)); k_tile = k_tile + 1) {
        {
            ...
            #pragma omp parallel sections
            {
                ...
                // load_Global<A_VECTOR_4, A_VECTOR_2, B_VECTOR_4, B_VECTOR_2, (THREADBLOCK_TILE_K * SPLIT_K - K_ > LOAD_K), false>(&shared_memory_A, &shared_memory_B, input_A, input_B, lda, ldb, cta_k, block_idx_x, block_idx_y, 1024 * (k_tile % 2), 1024 * (k_tile % 2));
                // load_Global<A_VECTOR_4_LAST, A_VECTOR_2_LAST, B_VECTOR_4, B_VECTOR_2, K_CHECK, THREADBLOCK_TILE_K_CHECK>(&shared_memory_A, &shared_memory_B, input_A, input_B, lda, ldb, cta_k, block_idx_x, block_idx_y, 1024 * (k_tile % 2), 1024 * (k_tile % 2));
...
__global__ void Thread_block_grid_1_1_3(const float * __restrict__ input_A, const dace::vec<float, 4> * __restrict__ input_B, float * __restrict__ output, int K, int M, int N) {
    constexpr long long VECLEN = 4;
    constexpr long long size_thread_block_tile_m = 128;
    constexpr long long size_thread_block_tile_n = 128;
    constexpr long long num_thread_blocks_m = 32;
