...
DACE_DFI void nested_nested_state_1_1_5(const dace::vec<float, 4> * input_A, const dace::vec<float, 4> * input_B, float * output, int K, int M, int N) {
...

    {
        ...
        #pragma omp parallel sections
        {
            #pragma omp section
            {
                dace::GlobalToShared2D<dace::vec<float, 4>, max(1, int_ceil(size_thread_block_tile_n, size_thread_tile_n)), max(1, int_ceil(size_thread_block_tile_m, size_thread_tile_m)), 1, size_thread_block_tile_m, size_K_tile / 4, 8 / 4, 1, true>(input_A, K / 4, 1, (dace::vec<float, 4> *)shared_memory_A);
            } // End omp section
            #pragma omp section
            {
                dace::GlobalToShared2D<dace::vec<float, 4>, max(1, int_ceil(size_thread_block_tile_n, size_thread_tile_n)), max(1, int_ceil(size_thread_block_tile_m, size_thread_tile_m)), 1, size_K_tile, size_thread_block_tile_n / 4, 128 / 4, 1, true>(input_B, N / 4, 1, (dace::vec<float, 4> *)shared_memory_B);
            } // End omp section
        } // End omp sections
    ...
    for (k_tile = 0; (k_tile < (num_K_tiles - 1)); k_tile = k_tile + 1) {
        {
            ...
            #pragma omp parallel sections
            {
                                    ...
                                    if (thread_i < size_thread_block_tile_m) {
                                        {
                                            for (auto k = 0; k < size_K_tile; k += 1) {
                                                ...
                                                shared_memory_B + (((128 * k) + thread_j) + (1024 * (k_tile % 2))), register_storage_B, 1);
                                                {
                                                    #pragma unroll
                ...
                #pragma omp section
                {
                    dace::GlobalToShared2D<dace::vec<float, 4>, max(1, int_ceil(size_thread_block_tile_n, size_thread_tile_n)), max(1, int_ceil(size_thread_block_tile_m, size_thread_tile_m)), 1, size_thread_block_tile_m, size_K_tile / 4, 8 / 4, 1, true>(input_A + (size_K_tile * (k_tile + 1)) / 4, K / 4, 1, ((dace::vec<float, 4> *)shared_memory_A) + (1024 * ((k_tile + 1) % 2)) / 4);
                } // End omp section
                #pragma omp section
                {
                    dace::GlobalToShared2D<dace::vec<float, 4>, max(1, int_ceil(size_thread_block_tile_n, size_thread_tile_n)), max(1, int_ceil(size_thread_block_tile_m, size_thread_tile_m)), 1, size_K_tile, size_thread_block_tile_n / 4, 128 / 4, 1, true>(input_B + ((N * size_K_tile) * (k_tile + 1)) / 4, N / 4, 1, ((dace::vec<float, 4> *)shared_memory_B) + (1024 * ((k_tile + 1) % 2)) / 4);
                } // End omp section
            } // End omp sections
            ...
            {
                {
                    ...
                    if (thread_j < size_thread_block_tile_n) {
                        if (thread_i < size_thread_block_tile_m) {
                            ...
                            register_storage_C, output + ((N * thread_i) + thread_j), [] (const float& x, const float& y) { return (x + y); }, N, 1);
                        }
                    }
                }
            }
    ...
    {
        {
            ...
            nested_nested_state_1_1_5(&input_A[((K * size_thread_block_tile_m) * thread_block_i) / 4], &input_B[(size_thread_block_tile_n * thread_block_j) / 4], &output[(((N * size_thread_block_tile_m) * thread_block_i) + (size_thread_block_tile_n * thread_block_j))], K, M, N);
        }
    }
}
