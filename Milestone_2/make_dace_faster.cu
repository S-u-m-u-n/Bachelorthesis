        ...
        for (int k = 0; k < LOAD_K; k++) {
            if (k % 2 == 0) {
                ...
                dace::CopyND<float, 1, false, size_thread_tile_m>::template ConstDst<1>::Copy(
                    A_Shared + (((k + ((8 * size_thread_tile_m) * bitwise_and(right_shift((thread % 32), 1), (warp_height - 1)))) + ((8 * size_warp_tile_m) * ((thread / 32) / num_warps_n))) + A_Shared_Offset_0), A_register_0, 1);
