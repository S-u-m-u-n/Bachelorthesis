...
#pragma unroll
    ...
    // dace::GlobalToShared2D<float, max(1, num_threads_per_threadblock), 1, 1, size_K_tile, size_thread_block_tile_n, 128, 1, true>(input_B + (K * cta_k) + (size_thread_block_tile_n * block_idx_x), N, 1, B_Shared + B_Shared_Offset_0);
        ...
        for (int k = 0; k < LOAD_K; k++) {
            if (k % 2 == 0) {
                ...
                dace::CopyND<float, 1, false, size_thread_tile_m / 2>::template ConstDst<1>::Copy(
                    A_Shared + (((k + ((8 * size_thread_tile_m) * bitwise_and(right_shift((thread % 32), 1), (warp_height - 1)))) + ((8 * size_warp_tile_m) * ((thread / 32) / num_warps_n))) + (1024 * (k_tile % 2))), register_storage_A, 128);
                ...
                // constexpr int M_THREADS = WARP_TILE_M / THREAD_TILE_M; // = 4


                // const int Shared_i = WarpIdy * WARP_TILE_M + i * M_THREADS * 4 + LaneIdy * 4; // i is either 0 or 1
                // const TYPE* shared_mem_pointer = &(*A_Shared)[A_Shared_Offset + Shared_i + THREADBLOCK_TILE_M * k];

                // A_Shared_Offset + Shared_i + THREADBLOCK_TILE_M * k

                // A_Shared + (((128 * k + ((8 * size_thread_tile_m / 2) * bitwise_and(right_shift((thread % 32), 1), (warp_height - 1)))) + ((8 * size_warp_tile_m) * ((thread / 32) / num_warps_n))) + A_Shared_Offset_0)
                // A_Shared + A_Shared_Offset_0 + 128 * k + ((8 * size_thread_tile_m / 2) * LaneIdy)) + ((8 * size_warp_tile_m) * WarpIdy
                // A_Shared + A_Shared_Offset_0 + 128 * k + WarpIdy * size_warp_tile_m + (4 * LaneIdy))
                ...
                // dace::CopyND<float, 1, false, size_thread_tile_n / 2>::template ConstDst<1>::Copy(
                    // B_Shared + ((((128 * k) + (size_thread_tile_n / 2 * LaneIdx)) + (size_warp_tile_n * WarpIdx)) + B_Shared_Offset_0) + 32, B_register_0 + 4, 1);
            } else {
