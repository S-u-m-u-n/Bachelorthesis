    ...
    {
        {
            ...
            nested_nested_state_1_1_5(&input_A[((K * size_thread_block_tile_m) * ((SWIZZLE * thread_block_i) + (thread_block_j % SWIZZLE))) / 4], &input_B[(size_thread_block_tile_n * (thread_block_j / SWIZZLE)) / 4], &output[(((N * ((size_thread_block_tile_m * ((SWIZZLE * thread_block_i) + (thread_block_j % SWIZZLE))) + (size_thread_tile_m * bitwise_and(right_shift(0, 1), (warp_height - 1))))) + (size_thread_block_tile_n * (thread_block_j / SWIZZLE))) + (size_thread_tile_n * bitwise_or(right_shift(bitwise_and(0, ((warp_height * warp_width) / 2)), (warp_width - 1)), bitwise_and(0, 1))))], K, M, N);
        }
    }
}
