...
constexpr long long ACCUMULATOR_SIZE = 2;
constexpr long long num_threads_per_thread_block = 256;
...
DACE_DFI void nested_nested_state_1_1_5(const float * input_A, const float * input_B, float * output, int K, int M, int N, long long thread_block_i, long long thread_block_j) {
    constexpr long long VECLEN = 4;
    constexpr float alpha = 1;
    constexpr float beta = 0;
    constexpr long long size_thread_block_tile_m = 128;
...

    {
        ...
        #pragma omp parallel sections
        {
            #pragma omp section
            {
                dace::GlobalToShared2D<float, max(1, num_threads_per_thread_block), 1, 1, size_thread_block_tile_m, size_K_tile, 8, 1, true>(input_A + (((K * size_thread_block_tile_m) * thread_block_i) + (size_K_tile * (num_K_tiles - 1))), K, 1, shared_memory_A + (1024 * ((num_K_tiles + 1) % 2)));
            } // End omp section
            #pragma omp section
            {
                dace::GlobalToShared2D<float, max(1, num_threads_per_thread_block), 1, 1, size_K_tile, size_thread_block_tile_n, 128, 1, true>(input_B + (((N * size_K_tile) * (num_K_tiles - 1)) + (size_thread_block_tile_n * thread_block_j)), N, 1, shared_memory_B + (1024 * ((num_K_tiles + 1) % 2)));
            } // End omp section
        } // End omp sections

    }

    for (k_tile = (num_K_tiles - 1); (k_tile > 0); k_tile = k_tile - 1) {
        {
            ...
            #pragma omp parallel sections
            {
                    ...
                    {
                        {
                            ...
                            if (thread < num_threads_per_thread_block) {
                                {
                                    for (auto k = 0; k < size_K_tile; k += 1) {

                                        dace::CopyND<float, 1, false, size_thread_tile_m>::template ConstDst<1>::Copy(
                                        shared_memory_A + (((k + ((8 * size_thread_tile_m) * ((thread % 32) / warp_width))) + ((8 * size_warp_tile_m) * ((thread / 32) / num_warps_n))) + (1024 * (k_tile % 2))), register_storage_A, 8);

                                        dace::CopyND<float, 1, false, size_thread_tile_n>::template ConstDst<1>::Copy(
                                        shared_memory_B + ((((128 * k) + (size_thread_tile_n * ((thread % 32) % warp_width))) + (size_warp_tile_n * ((thread / 32) % num_warps_n))) + (1024 * (k_tile % 2))), register_storage_B, 1);
                                        {
                                            #pragma unroll
                ...
                #pragma omp section
                {
                    dace::GlobalToShared2D<float, max(1, num_threads_per_thread_block), 1, 1, size_thread_block_tile_m, size_K_tile, 8, 1, true>(input_A + (((K * size_thread_block_tile_m) * thread_block_i) + (size_K_tile * (k_tile - 1))), K, 1, shared_memory_A + (1024 * ((1 - k_tile) % 2)));
                } // End omp section
                #pragma omp section
                {
                    dace::GlobalToShared2D<float, max(1, num_threads_per_thread_block), 1, 1, size_K_tile, size_thread_block_tile_n, 128, 1, true>(input_B + (((N * size_K_tile) * (k_tile - 1)) + (size_thread_block_tile_n * thread_block_j)), N, 1, shared_memory_B + (1024 * ((1 - k_tile) % 2)));
                } // End omp section
            } // End omp sections
    ...
    {
        {
            ...
            nested_nested_state_1_1_5(&input_A[0], &input_B[0], &output[0], K, M, N, thread_block_i, thread_block_j);
        }
    }
}
...
void __dace_runkernel_Thread_block_grid_1_1_3(gemm_t *__state, const float * __restrict__ input_A, const float * __restrict__ input_B, float * __restrict__ output, int K, int M, int N)
{
    ...
    cudaLaunchKernel((void*)Thread_block_grid_1_1_3, dim3(int_ceil(num_thread_blocks_n, 1), int_ceil(num_thread_blocks_m, 1), 1), dim3(max(1, num_threads_per_thread_block), 1, 1), Thread_block_grid_1_1_3_args, 0, __state->gpu_context->streams[0]);
}
