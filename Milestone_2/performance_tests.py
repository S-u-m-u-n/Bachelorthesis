...
if args.test == 1:
    ...
    subprocess.run(nvprof_options + [path + "double_buffering_swizzled_thread_blocks.csv"] + python_options + ["--double-buffering", "--swizzle-thread-blocks", "2"])
    subprocess.run(nvprof_options + [path + "double_buffering_swizzled_threads_swizzled_thread_blocks.csv"] + python_options + ["--double-buffering", "--swizzle-threads", "--swizzle-thread-blocks", "2"])
...
if args.test == 4:
    ...
    helpers.print_info("=" * 20, False)
    python_options += ["-M=256", "-N=256", "-K=10240", "--double-buffering", "--swizzle-threads"]
