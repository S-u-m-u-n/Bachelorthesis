...
if args.test == 2:
    ...
    helpers.print_info("=" * 20, False)
    subprocess.run(["nvprof", "--print-gpu-trace", "--csv", "--log-file", "1024_1024_unoptimized.csv", "python", "./sdfg_api.py", "-M=4096", "-N=4096", "-K=4096", "--repetitions=" + str(args.repetitions)])
    subprocess.run(["nvprof", "--print-gpu-trace", "--csv", "--log-file", "1024_1024_swizzled_threads.csv", "python", "./sdfg_api.py", "-M=4096", "-N=4096", "-K=4096", "--swizzle-threads", "--repetitions=" + str(args.repetitions)])
    subprocess.run(["nvprof", "--print-gpu-trace", "--csv", "--log-file", "1024_1024_cublas.csv", "python", "./cublas.py", "-M=4096", "-N=4096", "-K=4096", "--repetitions=" + str(args.repetitions)])
